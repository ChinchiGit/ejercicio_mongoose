...
const deleteProvider = async (req, res) => {
    try {
        ...
        if (buscar.length === 0) {
            ...
            res.status(200).json({message: `Se ha borrado el proveedor: ${nombreEmpresa}`});
        } else {
            res.status(409).json({message: `ERROR: El proveedor '${req.body.company_name}' no se puede borrar ya que tiene productos asociados en la Base de Datos.`});
        }
    } catch (error) {
        res.status(400).json({message: `ERROR: ${error.stack}`});
    }
